  ...
  if (JsonPreceptPaths->size() == 0) {
    try {
      for (const auto &entry : std::filesystem::recursive_directory_iterator(
               core::cJsonPreceptPath)) {
        if (entry.is_regular_file() &&
            entry.path().extension() == core::cJsonExtension) {
          std::cout << entry.path() << std::endl;
          JsonPreceptPaths->push_back(std::string(entry.path().c_str()));
        }
      }
    } catch (const std::filesystem::filesystem_error &e) {
      std::cerr << "Filesystem error at json precept: " << e.what()
                << std::endl;
...
void core::SaveLoadHandler::loadAllJsonScenePaths(
    std::vector<std::string> *JsonScenePaths) {
  if (JsonScenePaths->size() == 0) {
    try {
      for (const auto &entry : std::filesystem::recursive_directory_iterator(
               core::cJsonScenesPath)) {
        if (entry.is_regular_file() &&
            entry.path().extension() == core::cJsonExtension) {
          std::cout << entry.path() << std::endl;
...
void core::SaveLoadHandler::loadAllJsonPrecepts(
    ...
    try {
      for (const auto &entry : std::filesystem::recursive_directory_iterator(
        ...
        if (entry.is_regular_file() &&
            entry.path().extension() == core::cJsonExtension) {
          std::cout << entry.path() << std::endl;
          JsonPrecepts->emplace_back((core::JsonExtractor::loadJsonFromPath(
              std::string(entry.path()))));
        }
      }
    } catch (const std::filesystem::filesystem_error &e) {
      std::cerr << "Filesystem error at loadJsonPrecept: " << e.what()
                << std::endl;
    } catch (const std::exception &e) {
      std::cerr << "General error at loadJsonPrecept: " << e.what()
                << std::endl;
    }
  }
  // second load
    ...
    try {
      for (const auto &entry : std::filesystem::recursive_directory_iterator(
        ...
        if (entry.is_regular_file() &&
          ...
          for (GLuint i = 0; i < JsonPrecepts->size(); i++) {
            std::unique_ptr<nlohmann::json> jsonPtr(
                core::JsonExtractor::loadJsonFromPath(
                    std::string(entry.path())));
      ...
      std::cerr
          << "Filesystem error at core::SaveLoadHandler::loadAllJsonPrecepts: "
          << e.what() << std::endl;
    } catch (const std::exception &e) {
      std::cerr
          << "General error at core::SaveLoadHandler::loadAllJsonPrecepts: "
          << e.what() << std::endl;
    }
  }
}

void core::SaveLoadHandler::loadAllShaderPaths(
    std::vector<std::string> *ShaderPaths) {
  if (ShaderPaths->size() == 0) {
    try {
      for (const auto &entry :
           std::filesystem::recursive_directory_iterator(core::cShadersPath)) {
        if ((entry.is_regular_file() &&
             entry.path().extension() == core::cVertShaderExtension) ||
            (entry.is_regular_file() &&
             entry.path().extension() == core::cFragShaderExtension)) {
          std::cout << entry.path() << std::endl;
...
void core::SaveLoadHandler::loadAllModels(
    std::vector<std::unique_ptr<Model::Model>> *ObjectModels) {
  // the first load
  if (ObjectModels->size() == 0) {
    try {
      for (const auto &entry :
           std::filesystem::recursive_directory_iterator(core::cModelsPath)) {
        if ((entry.is_regular_file() &&
             entry.path().extension() == core::cModelObjExtension) ||
            (entry.is_regular_file() &&
             entry.path().extension() == core::cModelFBXExtension)) {
          std::cout << entry.path() << std::endl;
          ObjectModels->push_back(
              std::make_unique<Model::Model>(std::string(entry.path())));
        }
      }
    } catch (const std::filesystem::filesystem_error &e) {
      ...
      for (const auto &entry :
        ...
        if ((entry.is_regular_file() &&
          ...
          for (GLuint i = 0; i < ObjectModels->size(); i++) {
            if (ObjectModels->at(i).get()->directory ==
                std::string(entry.path().parent_path())) {
              std::cout << "SAME " << std::endl;
              break;
            } else if ((i == ObjectModels->size() - 1) &&
                       (ObjectModels->at(i).get()->directory !=
                        std::string(entry.path().parent_path()))) {
              ObjectModels->push_back(
                  std::make_unique<Model::Model>(std::string(entry.path())));
            }
...
void core::SaveLoadHandler::saveSceneAsJson(
    std::string inputFileName, GLboolean overwriteOption,
    std::vector<std::unique_ptr<Model::Model>> *ObjectModels,
  ...
  if (std::filesystem::exists(fullPath) && overwriteOption == false) {
    std::cerr
        << "File already exists (core::SaveLoadHandler::saveSceneAsJson) !"
        << std::endl;
  ...
  if (file.is_open()) {
    ...
    for (GLuint i = 0; i < Entities->size(); i++)
      newJsonFile["Entities"].push_back(Entities->at(i).get()->saveAsJson()),
          count = i + 1;
    ...
    for (GLuint i = 0; i < Lights->size(); i++)
      newJsonFile["Lights"].push_back(Lights->at(i).get()->saveAsJson()),
          count = i + 1;
    ...
    std::cerr
        << "Could not create file (core::SaveLoadHandler::saveSceneAsJson)"
        << std::endl;
    throw std::invalid_argument("SYSTEM::ERROR");
  }
}

void core::SaveLoadHandler::loadSceneFromJson(
    std::string filename,
    std::vector<std::unique_ptr<Model::Model>> *ObjectModels,
    std::vector<std::unique_ptr<Shader>> *Shaders,
    std::vector<std::unique_ptr<core::CoreEntity>> *Entities,
    std::vector<std::unique_ptr<Model::Light>> *Lights) {
  nlohmann::json *newJsonPtr = core::JsonExtractor::loadJsonFromPath(
      core::cJsonScenesPath + filename + cJsonExtension);
  ...
  for (GLuint i = 0; i < lightCount; i++) {
    ...
    newLight->light_color.x =
        newJson["Lights"][i]["Light Color"][0].get<GLfloat>();
    newLight->light_color.y =
        newJson["Lights"][i]["Light Color"][1].get<GLfloat>();
    newLight->light_color.z =
        newJson["Lights"][i]["Light Color"][2].get<GLfloat>();

    newLight->light_pos.x =
        newJson["Lights"][i]["Light Position"][0].get<GLfloat>();
    newLight->light_pos.y =
        newJson["Lights"][i]["Light Position"][1].get<GLfloat>();
    newLight->light_pos.z =
        newJson["Lights"][i]["Light Position"][2].get<GLfloat>();
  ...
  for (GLuint i = 0; i < modelCount; i++) {
    Model::Model *newModel =
        new Model::Model(newJson["Models"][i]["Path"].get<std::string>());
  ...
  for (GLuint i = 0; i < entityCount; i++) {
    ...
    newEntity->pos.x = newJson["Entities"][i]["Pos vector"][0].get<GLfloat>();
    newEntity->pos.y = newJson["Entities"][i]["Pos vector"][1].get<GLfloat>();
    newEntity->pos.z = newJson["Entities"][i]["Pos vector"][2].get<GLfloat>();

    newEntity->rotAxis.x =
        newJson["Entities"][i]["Rot vector"][0].get<GLfloat>();
    newEntity->rotAxis.y =
        newJson["Entities"][i]["Rot vector"][1].get<GLfloat>();
    newEntity->rotAxis.z =
        newJson["Entities"][i]["Rot vector"][2].get<GLfloat>();

    newEntity->rotDegreeRad =
        newJson["Entities"][i]["Rot degree"].get<GLfloat>();

    newEntity->modelScale.x =
        newJson["Entities"][i]["Model Scale"][0].get<GLfloat>();
    newEntity->modelScale.y =
        newJson["Entities"][i]["Model Scale"][1].get<GLfloat>();
    newEntity->modelScale.z =
        newJson["Entities"][i]["Model Scale"][2].get<GLfloat>();

    for (GLuint j = 0; j < ObjectModels->size(); j++) {
      if (newJson["Entities"][i]["Model Path"].get<std::string>() ==
          ObjectModels->at(j).get()->path) {
        newEntity->model = ObjectModels->at(j).get();
        break;
      } else {
        continue;
      }
