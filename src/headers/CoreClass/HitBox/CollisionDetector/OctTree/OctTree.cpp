...
// ONLY USE THIS AT 0 NODE
void core::OctTreeNode::insertEntity(core::CoreEntity *entity) {
  //
  glm::vec3 relativePos = entity->pos - this->Box.position;
  // Dont try to hurt your head here buckeroo :sunglasses_dumb_smirk_emote_here:
  // Only schizos understands this I assure thee
  GLint xdir = relativePos.x <= 0 ? 0 : 4;
  GLint ydir = relativePos.y <= 0 ? 0 : 2;
  GLint zdir = relativePos.z <= 0 ? 0 : 1;
  GLint dir = (xdir + ydir + zdir);
  if (this->Directions.at(dir)->heldEntities->empty()) {
    this->Directions.at(dir)->
  } else {
    // Resolve things here dammit
  };
  //
}
