...
core::Window::Window(std::string windowName, WindowType type, GLuint SCR_WIDTH,
                     GLuint SCR_HEIGHT)
...
core::Window::~Window() { this->destroyWindow(); }

void core::Window::createWindow() {
  if (windowIsSet == 0) {
    std::cout << "Creating window at core::Window::createWindow()" << std::endl;
    switch (this->type) {
    case FULL_SCREEN:
      this->window =
          glfwCreateWindow(this->SCR_WIDTH, this->SCR_HEIGHT,
                           windowName.c_str(), glfwGetPrimaryMonitor(), NULL);
      // checks if windows is created succesfully
      if (this->window == NULL) {
        std::cout
            << "Failed to create GLFW window at core::Window::createWindow()"
            << std::endl;
    ...
    default:
      throw std::invalid_argument(
          "No proper window type was given at core::Window::createWindow()");
      break;
    }
    this->windowIsSet = 1;
  } else {
...
void core::Window::destroyWindow() {
...
const GLuint &core::Window::getScrWidth() const { return this->SCR_WIDTH; };

const GLuint &core::Window::getScrHeight() const { return this->SCR_WIDTH; };

GLFWwindow *core::Window::returnGLFWWindow() { return this->window; }
