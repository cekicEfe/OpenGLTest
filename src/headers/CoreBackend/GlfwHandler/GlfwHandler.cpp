...
void core::GlfwHandler::initGlfwHandler(std::string windowName,
  ...
  if (!glfwInit()) {
    std::cerr
        << "GLFW initialization failed at core::GlfwHandler::initGlfwHandler!"
        << std::endl;
  ...
  std::cout << "Setting up callbacks at core::GlfwHandler::initGlfwHandler"
            << std::endl;
  this->inputHandler.setupCallbacks(this->windowHandler.returnMainGLFWWindow());
...
GLuint core::GlfwHandler::checkWindowShouldClose() {
  return glfwWindowShouldClose(this->windowHandler.returnMainGLFWWindow());
}

void core::GlfwHandler::beginLoop() {
  this->setLoopVariables();
  this->initLoopImGui();
  ImGui::ShowDemoWindow();
}

GLFWwindow *core::GlfwHandler::returnMainWindow() {
  return this->windowHandler.returnMainGLFWWindow();
}

void core::GlfwHandler::endLoop() {
  this->renderImGui();
  this->swapBuffers();
  this->pollEvents();
}

void core::GlfwHandler::cleanupGlfwHandler() {
...
void core::GlfwHandler::pollEvents() { glfwPollEvents(); }

void core::GlfwHandler::initImGui() {
...
void core::GlfwHandler::initLoopImGui() { this->imguiHandler.initLoop(); }

void core::GlfwHandler::renderImGui() { this->imguiHandler.render(); }

void core::GlfwHandler::terminateImGui() {
