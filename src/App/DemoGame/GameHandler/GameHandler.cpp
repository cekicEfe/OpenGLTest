...
void
testgame::GameHandler::demoShowGui ()
{
  ...
  auto preparePicker = [] (ImGui::FileBrowser &browser,
  ...
  auto createPicker = [] (ImGui::FileBrowser &browser,
                          std::string &filename,
                          fs::path &path,
                          bool &cond) {
                ...
                for (size_t i = 0; i < this->models.size (); i++)
                  {
                    ...
                    else if (this->models[i].get ()->getModelName ()
                                 != model_name
                ...
                if (!model_already_exists)
                  {
                    this->models.push_back (
                        std::make_shared<testgame::GameModel> (
            ...
            if (fs::exists (frag_path) && fs::exists (vert_path))
              {
                if (frag_name.substr (0, frag_name.find_last_of ("."))
                  ...
                  {
                    shader_name
                        = frag_name.substr (0, frag_name.find_last_of ("."));
                    for (size_t i = 0; i < this->shaders.size (); i++)
                      {
                        if (this->shaders[i].get ()->getShaderName ()
                            == shader_name)
                          {
                    ...
                    try
                      {
                        this->shaders.push_back (
                            std::make_shared<testgame::GameShader> (
                                vert_path.c_str (),
                                frag_path.c_str (),
                                shader_name));
        ...
        for (size_t i = 0; i < this->shaders.size (); i++)
          {
            ...
            std::string shader_name
                = "Select shader : "
                  + this->shaders[i].get ()->getShaderName ();
...
void
testgame::GameHandler::demoMainLoop (const core::Window &window)
{
...
GLfloat
testgame::GameHandler::calculateDeltaTime ()
{
...
void
testgame::GameHandler::update (GLfloat deltaTime)
{
}

void
testgame::GameHandler::processInput (GLFWwindow *window, GLfloat deltaTime)
{
...
void
testgame::GameHandler::mouse_callback (GLFWwindow *window, double xposIn,
                                       double yposIn)
{
...
void
testgame::GameHandler::defaultFramebufferSizeCallback (GLFWwindow *window,
...
void
testgame::GameHandler::scrollCallback (GLFWwindow *window, double xoffset,
                                       double yoffset)
{
  if (!testgame::GameHandler::menuIsUp)
